...
describe("test smart contract interactor", function () {
    ...
    it("should interact with 'answer'", async function () {
        ...
        assert.equal(transaction.getHash().toString(), "5c0d6772a47ad77b9747cde2c70ab41f3dbcf83d6405b086d2a43767a0a87156");

        transaction = await runner.run(interaction.withNonce(new Nonce(1)));
        assert.equal(transaction.getNonce().valueOf(), 1);
        assert.equal(transaction.getHash().toString(), "3e679b1999bc425105b61455a3b23a5f13becc91357b2234ad548d60df60739f");
    ...
    it("should interact with 'lottery_rewa'", async function () {
        setupUnitTestWatcherTimeouts();

        let abiRegistry = await loadAbiRegistry(["src/testdata/lottery_rewa.abi.json"]);
        ...
        let startInteraction = <Interaction>contract.methods.start([
            ...
            new BigUIntValue(Balance.rewa(1).valueOf()),
